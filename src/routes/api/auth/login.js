...
export const post = async ({body}) => {
    ...
    const user = await connection.promise().query("SELECT * FROM users WHERE email = ?", [
        body.email
    ]);
    ...
    const duplicateUser = await connection.promise().query("SELECT * FROM cookies WHERE email= ?", [
        body.email
    ]);

    // if there is an user with cookies, update the cookies, otherwise, create new cookie db entry
    if(duplicateUser) {
        await connection.promise().query("UPDATE cookies SET cookieId = ? WHERE email = ?", [
            cookieId,
            body.email
        ]);
    } else {
        await connection.promise().query('INSERT INTO cookies (email, cookie) VALUES (?,?)', [
            body.email,
            cookieId
        ]);
    }
